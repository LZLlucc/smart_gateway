...
static int write_task_func(void *argv)
{
	...
	char datas_buf[128] = {0};
	int len =
		app_buffer_read(device->down_buffer, datas_buf, sizeof(datas_buf));
	if (-1 == len) {
		log_error("app_buffer_read failed");
		return -1;
	}
	...
	if (len > 0 && device->pre_write) {
		...
		if (len > 0) {
			/* 如果间隔时间小于200ms，等待 */
			long long distance_time =
				get_current_time() - device->last_write_time;
...
{
	if (!device->is_running) {
		log_info("device is already closed");
		return;
	}
	device->is_running = 0;
	pthread_cancel(device->read_pthread);
	pthread_join(device->read_pthread, NULL);
	app_buffer_free(device->up_buffer);
	app_buffer_free(device->down_buffer);
	free(device->post_read);
	free(device->pre_write);
    close(device->fd);
	free(device);
	device = NULL;
}